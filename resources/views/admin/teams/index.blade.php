...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <a href="{{ route("admin.teams.create") }}" class="btn btn-success">Create Team</a>
        </div>
        {{-- <div class="card-body"> --}}
        <table id="categories" class="table table-bordered table-striped">
            ...
            @foreach ($teams as $team)
                ...
                <tr>
                    <td>{{ $team->id }}</td>
                    <td>{{ $team->localizations[0]->name }}</td>
                    <td>{{ $team->localizations[0]->position }}</td>
                    <td>{{ $team->localizations[0]->text }}</td>
                    <td>
                        @php
                          ...
                          if ($extension === 'gif') {
                             $imgUrl = $team->getFirstMediaUrl('teams');
                              ...
                              $imgUrl = $team->getFirstMediaUrl('teams','mobile');
                          }
                        @endphp
